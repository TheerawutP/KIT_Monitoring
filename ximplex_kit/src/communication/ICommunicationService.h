...
public:
   ...
   */
  virtual void setCommandCallback(std::function<void(const char *id, const char *command, const char *data)> callback) = 0;

  /**
   * @brief Acknowledge a command execution result
   * @param commandId Unique ID of the command
   * @param status Result status ("SUCCESS", "ERROR", etc.)
   */
  virtual void acknowledgeCommand(const char *commandId, const char *status) = 0;
};
