...
public:
    ...
    bool isConnected();
    void setCommandCallback(std::function<void(const char* command, const char* data)> callback);
    void clearCommand();
...
private:
    ...
    // Command handling
    std::function<void(const char* command, const char* data)> m_commandCallback;
